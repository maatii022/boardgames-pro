...
export default function SalaJugador() {
  ...
  // Al montar: pedir el estado actual por si los eventos llegaron antes que los listeners
  useEffect(() => {
    emitir('pedir-estado');
  }, []);
  ...
  useEffect(() => {
    const c1 = escuchar('sala-actualizada', (s) => {
      setSala(s);
      setFase(s.fase);
    });
    const c2 = escuchar('estado-actualizado', (e) => {
      setEstado(e);
      if (e?.fase) setFase(e.fase);
    });
    ...
    const c6 = escuchar('expulsado', ({ mensaje }) => {
      alert(mensaje);
      navigate('/');
    });
        ...
        <div style={{ maxWidth: '420px', margin: '0 auto' }}>
          ...
          <div style={{ background: 'rgba(13,27,46,0.8)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '12px', padding: '20px', marginBottom: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <h2 style={{ fontFamily: 'var(--fuente-subtitulo)', color: 'var(--crema-pergamino)', fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase' }}>
                Tripulación
              </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                ...
                return (
                  <div key={j.id || i} style={{
                    ...
                    <div style={{
                      ...
                      background: esHost ? 'linear-gradient(135deg,#c9a84c,#e8c97a)' : 'rgba(26,58,92,0.8)',
                    ...
                    {/* Botones solo visibles para el Host (sobre otros jugadores) */}
                    {soyHost && !esYo && (
                      <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
                        {!esHost && (
                          <button onClick={() => cambiarHost(j.id)} title="Hacer Host" style={{
                            background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.25)',
                            color: 'var(--oro-dorado)', borderRadius: '4px', padding: '3px 7px',
                            fontSize: '10px', fontFamily: 'var(--fuente-subtitulo)',
                            cursor: 'pointer', letterSpacing: '1px',
                          }}>host</button>
                        )}
                        <button onClick={() => kickJugador(j.id)} title="Expulsar" style={{
                          background: 'rgba(192,57,43,0.12)', border: '1px solid rgba(192,57,43,0.3)',
                          color: '#ff8a8a', borderRadius: '4px', padding: '3px 7px',
              ...
              {numJugadores === 0 && (
                <p style={{ fontFamily: 'var(--fuente-cuerpo)', color: 'rgba(245,230,200,0.25)', fontSize: '14px', textAlign: 'center', padding: '20px 0' }}>
                  Esperando jugadores...
                </p>
              )}
            </div>
          ...
          {soyHost ? (
            <div style={{ background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.25)', borderRadius: '12px', padding: '20px' }}>
              <p style={{ fontFamily: 'var(--fuente-subtitulo)', color: 'var(--oro-dorado)', fontSize: '10px', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '14px', textAlign: 'center' }}>
                ⚓ Eres el Host
              </p>
              {numJugadores < 5 && (
              ...
              <button className="btn-primario" onClick={() => emitir('host-iniciar-partida')}
                disabled={numJugadores < 5} style={{ width: '100%', padding: '16px', fontSize: '15px' }}>
                🎮 Iniciar Partida
              </button>
            </div>
          ) : (
            <div style={{ background: 'rgba(10,147,150,0.06)', border: '1px solid rgba(10,147,150,0.2)', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '12px' }}>
                {[0,1,2].map(i => (
                  <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--turquesa-kraken)', animation: `pulsar-kraken 1.4s ease-in-out ${i*0.25}s infinite` }} />
                ))}
              </div>
              <p style={{ fontFamily: 'var(--fuente-subtitulo)', color: 'rgba(245,230,200,0.45)', fontSize: '13px', letterSpacing: '1px' }}>
      ...
      <div className="fondo-mar movil-scroll" style={{ width: '100%', minHeight: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
        <div className="aparecer" style={{ width: '100%', maxWidth: '380px', textAlign: 'center' }}>
          ...
          <div style={{ background: rolCfg?.bg, border: `2px solid ${rolCfg?.borde}`, borderRadius: '20px', padding: '36px 28px', boxShadow: `0 0 60px ${rolCfg?.color}40`, marginBottom: '28px' }}>
            ...
            <h1 style={{ fontFamily: 'var(--fuente-titulo)', fontSize: '28px', color: rolCfg?.color, textShadow: `0 0 30px ${rolCfg?.color}80`, letterSpacing: '4px', marginBottom: '6px' }}>
              {rolCfg?.nombre?.toUpperCase()}
            </h1>
            {miJugador.personaje && (
              <>
    ...
    return (
      <div className="fondo-mar movil-scroll" style={{ width: '100%', minHeight: '100%', padding: '24px 16px 40px' }}>
        <div style={{ maxWidth: '400px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <VotacionMotin pistolas={miJugador?.pistolas ?? 3} umbral={estadoJugador?.motin?.umbral} confirmados={estadoJugador?.motin?.confirmados} total={jugadores.length} emitir={emitir} />
        </div>
      </div>
    );
  }
